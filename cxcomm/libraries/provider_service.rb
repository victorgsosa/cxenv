...
module CXCommerceCookbook
	class CXCommerceCookbook::ServiceProvider < Chef::Provider::LWRPBase
	  ...
	  def action_configure
	    ...
	    install = find_cxcomm_resource(Chef.run_context, :cxcomm_install, new_resource)
	    default_config_name = new_resource.service_name || 'cxcomm'

	    d_r = directory "/var/run/cxcomm" do
	      owner user.user
	      group user.group
	      mode '0755'
	      recursive true
	    ...
	    if new_resource.systemd_source
	      ...
	      systemd_r = template "/usr/lib/systemd/system/#{new_resource.service_name}.service" do
	        ...
	        variables(
	          ...
	          path_home: "#{install.dir}/hybris/installer",
	          user: user.user,
	          group: user.group,
